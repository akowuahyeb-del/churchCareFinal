...
export default function HomeScreen({ navigation }) {
  ...
  const createEvent = () => {
    ...
    setEvents(prev => [{
      ...
      date: newDate, desc: newDesc, active: true, expiry: null
    }, ...prev]);
  ...
  const handleUpload = () => {
    Alert.alert("Upload Flyer", "Choose a source", [
      { text: "Photo Library", onPress: async () => {
          ...
          if (!result.canceled)
            setFlyers(prev => [...prev, { id: Date.now().toString(), image: { uri: result.assets[0].uri }, active: true, expiry: null }]);
      }},
  ...
  /* ─────────────────────────────────────────────────────────────
     MEMBERS NAVIGATION
     Navigator (from App.js in this thread):
       RootStack
         └── MainTabs  (Tab.Navigator)
               ├── Home      → HomeScreen          ← we are here
               ├── Members   → MembersStack
               │     └── MembersMain → MembersScreen   ← target
               ├── Attendance
               ├── History
               └── Settings

     jumpTo("Members") switches the Members tab.
     Passing { screen: "MembersMain" } ensures MembersScreen
     is always shown, even if the stack previously navigated deeper.
  ───────────────────────────────────────────────────────────── */
  const goToMembers = () => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      scrollRef.current?.scrollTo({
        x: currentIndex.current * (screenWidth - 30), animated: true
      });
    }, 3000);
    ...
    <View style={{ flex: 1, backgroundColor: "#f4f6fb", paddingBottom: 10 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
          ...
          {manageFlyersExpanded && flyers.map(item => (
            <View key={item.id} style={styles.adminRow}>
              ...
              <View style={{ flex: 1, marginLeft: 8 }}>
                ...
                {item.expiry && (
                  <Text style={styles.expiryBadge}>⏱ {fmtDateTime(item.expiry)}</Text>
                )}
              </View>

              <TouchableOpacity
                style={[styles.adminBtn, { backgroundColor: "#6c47b8" }]}
              ...
              <TouchableOpacity
                style={[styles.adminBtn, { backgroundColor: item.active ? "#ff4d4d" : "#1BA97F", marginLeft: 4 }]}
        ...
        <View style={styles.quickGrid}>
          ...
          {/* ✅ Points to MembersScreen via MembersStack > MembersMain */}
          <TouchableOpacity
              ...
              {activeEvents.map(event => (
                <View key={event.id} style={styles.eventCard}>
                  <View style={{ flex: 1 }}>
                    ...
                    {event.expiry && (
                      <Text style={styles.expiryBadge}>⏱ {fmtDateTime(event.expiry)}</Text>
                    )}
                  </View>
      ...
      <Modal visible={createModalVisible} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ccc", marginTop: 6 }]} onPress={() => setCreateModalVisible(false)}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View></View>
      </Modal>
      ...
      <Modal visible={editModalVisible} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ccc", marginTop: 6 }]} onPress={() => setEditModalVisible(false)}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View></View>
      </Modal>
      ...
      <Modal visible={deleteModalVisible} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ccc", marginTop: 6 }]} onPress={() => setDeleteModalVisible(false)}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View></View>
      </Modal>
      ...
      <Modal visible={deleteFlyerModalVisible} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          <Text style={{ color: "#666", marginBottom: 14, textAlign: "center" }}>
            This flyer will be permanently removed.
          </Text>
          {flyerToDelete && <Image source={flyerToDelete.image} style={styles.flyerPreview} />}
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ff4d4d", marginTop: 12 }]} onPress={confirmDeleteFlyer}>
            <Text style={{ color: "#fff" }}>Yes, Delete</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ccc", marginTop: 6 }]} onPress={() => setDeleteFlyerModalVisible(false)}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View></View>
      </Modal>
      ...
      <Modal visible={flyerExpiryModal} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          {showFlyerDatePicker && (
            <DateTimePicker
              value={flyerExpiryDate} mode="date"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowFlyerDatePicker(false);
                if (d) setFlyerExpiryDate(prev => {
                  const n = new Date(d); n.setHours(prev.getHours(), prev.getMinutes()); return n;
                });
              }}
            />
          )}
          ...
          <TouchableOpacity style={styles.pickerRow} onPress={() => setShowFlyerTimePicker(true)}>
            ...
            <Text style={styles.pickerValue}>
              {flyerExpiryDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </Text>
          </TouchableOpacity>
          {showFlyerTimePicker && (
            <DateTimePicker
              value={flyerExpiryDate} mode="time"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowFlyerTimePicker(false);
                if (d) setFlyerExpiryDate(prev => {
                  const n = new Date(prev); n.setHours(d.getHours(), d.getMinutes()); return n;
                });
              }}
            />
          )}

          <TouchableOpacity style={[styles.modalBtn, { marginTop: 14 }]} onPress={saveFlyerExpiry}>
            <Text style={{ color: "#fff" }}>Save Expiry</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#e74c3c", marginTop: 6 }]} onPress={clearFlyerExpiry}>
            <Text style={{ color: "#fff" }}>Clear Expiry</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ccc", marginTop: 6 }]} onPress={() => setFlyerExpiryModal(false)}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View></View>
      </Modal>
      ...
      <Modal visible={eventExpiryModal} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          {showEventDatePicker && (
            <DateTimePicker
              value={eventExpiryDate} mode="date"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowEventDatePicker(false);
                if (d) setEventExpiryDate(prev => {
                  const n = new Date(d); n.setHours(prev.getHours(), prev.getMinutes()); return n;
                });
              }}
            />
          ...
          <TouchableOpacity style={styles.pickerRow} onPress={() => setShowEventTimePicker(true)}>
            <Ionicons name="time-outline" size={16} color="#4B3F72" />
            <Text style={styles.pickerValue}>
              {eventExpiryDate.toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
            </Text>
          </TouchableOpacity>
          {showEventTimePicker && (
            <DateTimePicker
              value={eventExpiryDate} mode="time"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
          ...
          <TouchableOpacity style={[styles.modalBtn, { marginTop: 14 }]} onPress={saveEventExpiry}>
            <Text style={{ color: "#fff" }}>Save Expiry</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#e74c3c", marginTop: 6 }]} onPress={clearEventExpiry}>
            <Text style={{ color: "#fff" }}>Clear Expiry</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.modalBtn, { backgroundColor: "#ccc", marginTop: 6 }]} onPress={() => setEventExpiryModal(false)}>
            <Text>Cancel</Text>
          </TouchableOpacity>
        </View></View>
      </Modal>
