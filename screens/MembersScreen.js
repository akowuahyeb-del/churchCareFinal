...
const [loadingMembers, setLoadingMembers] = useState(false);
  ...
  try {
    ...
    // ✅ Save to cache
    await AsyncStorage.setItem(MEMBERS_CACHE_KEY, JSON.stringify(freshData));

  } catch (err) {
    console.log("Network error → loading cache", err);
    ...
    try {
      ...
      if (cached) {
        ...
        setError("Showing offline data. Connect to internet for latest updates.");
      } else {
        setError("We’re unable to load members right now. Please check your internet.");
      }
  ...
  {/* ✅ INSERTED DONATE HEADER BUTTON */}
  <TouchableOpacity
    style={styles.donateHeaderBtn}
    onPress={() => navigation.getParent()?.navigate("Donate")}
  >
