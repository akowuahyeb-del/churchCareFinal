...
export default function HomeScreen({ navigation }) {
  ...
  const saveEdit = () => {
    setEvents(prev => prev.map(e => e.id === editingEvent.id ? { ...e, title: editTitle, date: editDate, desc: editDesc } : e));
  ...
  /* ─────────────────────────────────────────────────────────
     ✅ MEMBERS NAVIGATION — FIXED
     Navigator structure:
       RootStack > MainTabs (Tab) > Members tab > MembersStack > MembersMain
     From HomeScreen (inside MainTabs), jumpTo("Members") switches
     the tab. The stack inside then shows MembersMain automatically.
  ───────────────────────────────────────────────────────── */
  const goToMembers = () => {
    navigation.jumpTo("Members");
  };
  ...
  return (
    <View style={{ flex: 1, backgroundColor: "#f4f6fb", paddingBottom: 10 }}>
      ...
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 120 }}>
        ...
        {/* ── 4. QUICK ACTIONS — colorful modern cards ── */}
        ...
        <View style={styles.quickGrid}>
          ...
          {/* ✅ FIXED — jumpTo("Members") is correct for tab screens */}
      ...
      <Modal visible={flyerExpiryModal} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          {showFlyerDatePicker && (
            <DateTimePicker value={flyerExpiryDate} mode="date"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowFlyerDatePicker(false);
                if (d) setFlyerExpiryDate(prev => { const n = new Date(d); n.setHours(prev.getHours(), prev.getMinutes()); return n; });
              }} />
          )}
          ...
          {showFlyerTimePicker && (
            <DateTimePicker value={flyerExpiryDate} mode="time"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowFlyerTimePicker(false);
                if (d) setFlyerExpiryDate(prev => { const n = new Date(prev); n.setHours(d.getHours(), d.getMinutes()); return n; });
              }} />
          )}
      ...
      <Modal visible={eventExpiryModal} transparent animationType="fade">
        <View style={styles.modalWrap}><View style={styles.modalBox}>
          ...
          {showEventDatePicker && (
            <DateTimePicker value={eventExpiryDate} mode="date"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowEventDatePicker(false);
                if (d) setEventExpiryDate(prev => { const n = new Date(d); n.setHours(prev.getHours(), prev.getMinutes()); return n; });
              }} />
          )}
          ...
          {showEventTimePicker && (
            <DateTimePicker value={eventExpiryDate} mode="time"
              display={Platform.OS === "ios" ? "spinner" : "default"}
              onChange={(e, d) => {
                setShowEventTimePicker(false);
                if (d) setEventExpiryDate(prev => { const n = new Date(prev); n.setHours(d.getHours(), d.getMinutes()); return n; });
              }} />
          )}
...
const styles = StyleSheet.create({
  ...
  /* ✅ Modern colorful quick action cards */
